...
![image-20200203181455269](C:\Users\Owner\AppData\Roaming\Typora\typora-user-images\image-20200203181455269.png)
...
1.备份数据库或表
	#压缩备份数据库
	mysqldump --databases mysql -uroot -proot123 | gzip > /data/mysqldump/mysql_$(date +%F).gz
	#仅压缩备份数据库结构
	mysqldump --no-data --databases mysql -uroot -proot123 | gzip > /data/mysqldump/mysql_$(date +%F).gz
	#不压缩备份数据库
	mysqldump --databases mysql -uroot -proot123 > mysql_bak.sql
	#备份某个（些）表	#（备份mysql库吓得两个表）
	mysqldump -uroot -proot123 mysql user db > user_db.sql
	#仅备份某个（些）表结构
