...
如果你因为丢失套接字文件而不能连接，你可以简单地通过重启服务器重新创建得到它。因为服务器在启动时重新创建它。
    #mysqldump不锁表备份
    /bin/mysqldump -uroot -ptop123456 --socket=/usr/local/mysql/mysql.sock --set-gtid-purged=OFF --skip-opt --triggers --routines --events -A --single-transaction --master-data=2 |/bin/gzip >
